...
\begin{titlepage}
    \begin{center}
        ...
        {\large на тему}\\
        {\large\bfseries «ДИСКРЕТНЫЙ ЭЛЕМЕНТ В НЕПРЕРЫВНОЙ СИСТЕМЕ. УСТОЙЧИВОСТЬ ДИСКРЕТНЫХ СИСТЕМ. ПОСТРОЕНИЕ ДИСКРЕТНЫХ КОМАНДНЫХ ГЕНЕРАТОРОВ»}\\
        Вариант 20
        \vfill
...
$$
\det{\left( z I-F \right)}=z^2+z\left( T^2k_1/2+Tk_2-2 \right)+\left( T^2k_1/2-Tk_2+1 \right)=0
$$


Пусть желаемый полином:
$$
\left( z-z_1 \right)\left( z-z_2 \right)=z^2-\left( z_1+z_2 \right)z+z_1z_2
$$


Для спектра $\sigma_1=\left\{ 1.59, 1.02 \right\}$:
$$
z^2-\left( 1.59+1.02 \right)z+1.59\cdot1.02=z^2-2.61z+1.6218
$$
...
$$
z^2-\left( -0.08-0.69 \right)z+0.08\cdot0.69=z^2+0.77z+0.0552,
$$
$$
\begin{cases}
...
$$
z^2-\left( 0.29-0.06 \right)z-0.29\cdot0.06=z^2-0.23z-0.0174,
$$
$$
\begin{cases}
...
$$
z^2-\left( 0.31j -0.37j \right)z-0.31j\cdot0.37j=z^2+0.06jz+0.1147,
$$
$$
\begin{cases}
...
$$
z^2-\left( -1.35+1.02j-1.35-1.02j \right)z+\left( -1.35+1.02j \right)\left( -1.35-1.02j \right),
$$
$$
z^2+2.7z+2.8629
$$
...
Когда модуль собственных чисел больше 1, то система расходится.
...
\begin{align*}
    ...
    &A=-8.5,\\
    &\omega=0.7,\\
    &f\left( k \right)=0.4\sin{\left( 2kT \right)}+2.5\cos{\left( 6kT \right)},
\end{align*}
где $f\left( k \right)$ -- внешнее возмущение.
...
\begin{cases}
    ...
    \xi_2\left( k+1 \right)=-\xi_1\left( k \right)+2\cos{\left( T\omega \right)}\xi_2\left( k \right),\\
...
$$
g\left( k \right)=f\left( k \right)=0.4\sin{\left( 2kT \right)}+2.5\cos{\left( 6kT \right)}
$$


Делим $g\left( k \right)$ на две подсистемы:
$$
g\left( k \right)=g_1\left( k \right)+g_2\left( k \right)
$$


Аналогично общему случаю:
$$
\xi_1\left( k \right)=g_1\left( k \right),\ \eta_1\left( k \right)=g_2\left( k \right);
$$
\begin{align*}
    \xi_2\left( k \right)&=\xi_1\left( k+1 \right)=0.4\sin{\left( 2kT+2T \right)}=\\
    &=\xi_1\left( k \right)\cos{\left( 2T \right)}+0.4\cos{\left( 2kT \right)}\sin{\left( 2T \right)},
\end{align*}
\begin{align*}
    \eta_2\left( k \right)&=\eta_1\left( k+1 \right)=2.5\cos{\left( 6kT+6T \right)}=\\
    &=\eta_1\left( k \right)\cos{\left( 6T \right)}-2.5\sin{\left( 6kT \right)}\sin{\left( 6T \right)};
\end{align*}
\begin{align*}
    \xi_3\left( k \right)&=\xi_2\left( k+1 \right)=\\
    &=\xi_2\left( k \right)\cos{\left( 2T \right)}+0.4\cos{\left( 2kT+2T \right)}\sin{\left( 2T \right)}=\\
    &=2\xi_2\left( k \right)\cos{\left( 2T \right)}-\xi_1\left( k \right),
\end{align*}
\begin{align*}
    \eta_3\left( k \right)&=\eta_2\left( k+1 \right)=\\
    &=\eta_2\left( k \right)\cos{\left( 6T \right)}-2.5\sin{\left( 6kT +6T \right)}\sin{\left( 6T \right)}=\\
    &=2\eta_2\left( k \right)\cos{\left( 6T \right)}-\eta_1\left( k \right);
\end{align*}
...
\begin{cases}
    \xi_1\left( k+1 \right)=\xi_2\left( k \right),\\
    \xi_2\left( k+1 \right)=-\xi_1\left( k \right)+2\cos{\left( 2T \right)}\xi_2\left( k \right),\\
    \eta_1\left( k+1 \right)=\eta_2\left( k \right),\\
    \eta_2\left( k+1 \right)=-\eta_1\left( k \right)+2\cos{\left( 6T \right)}\eta_2\left( k \right),\\
    g\left( k \right)=\xi_1\left( k \right)+\eta_1\left( k \right)
\end{cases}
$$
...
$$
\psi\left( 0 \right)=\begin{bmatrix}
    \xi_1\left( 0 \right)\\
    \xi_2\left( 0 \right)\\
    \eta_1\left( 0 \right)\\
    \eta_2\left( 0 \right)
\end{bmatrix}=\begin{bmatrix}
    0\\ 0.4\sin{\left( 2T \right)}\\2.5\\2.5\cos{\left( 6T \right)}
\end{bmatrix}
$$
...
\begin{bmatrix}
    \xi_1\left( k+1 \right)\\
    \xi_2\left( k+1 \right)\\
    \eta_1\left( k+1\right)\\
    \eta_2\left( k+1 \right)
\end{bmatrix}=\begin{bmatrix}
    0&1&0&0\\
    -1&2\cos{\left( 2T \right)}&0&0\\
    0&0&0&1\\
    0&0&-1&2\cos{\left( 6T \right)}
\end{bmatrix}\begin{bmatrix}
    \xi_1\left( k \right)\\
    \xi_2\left( k \right)\\
    \eta_1\left( k\right)\\
    \eta_2\left( k \right)
\end{bmatrix}=\Gamma_d\,\psi\left( k \right),
$$
$$
g\left( k \right)=\begin{bmatrix}
    1&0&1&0
\end{bmatrix}\begin{bmatrix}
    ...
    \eta_1\left( k\right)\\
    \eta_2\left( k \right)
\end{bmatrix}=H\psi\left( k \right)
$$

\newpage
Схема моделирования генератора возмущений:
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.55]{3_2.png}
    \caption{Схема генератора возмущений}
    \label{fig:3_2}
\end{figure}


Выполним моделирование генератора возмущений:
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.8]{3_21.png}
    \caption{Выход генератора $g_1\left( k \right)$}
    \label{fig:3_21}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.8]{3_22.png}
    \caption{Выход генератора $g_2\left( k \right)$}
    \label{fig:3_22}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.8]{3_23.png}
    \caption{Выход генератора $g\left( k \right)$}
    \label{fig:3_23}
\end{figure}
...
было исследовано влияние дискретного
элемента на непрерывную систему --
был сделан вывод о снижении запаса
устойчивости. Исследование
устойчивости дискретных систем
показало, что все собственные числа
системы внутри единичного круга
гарантируют устойчивость системы.
Был синтеирован командный генератор
и генератор возмущений. Моделирование
показало, что таким способом
успешно получается моделировать
гармонические сигналы без дифференцирования.
\end{document}